'''
Script removes from parsed, tokenized, aligned datasets items that were parsed by stanza 
...
def find_sent_to_remove_file(pipeline, lang):
    file = "./data/" + pipeline + "/parsed/" + \
        pipeline + "." + lang + ".parsed.conllu"
    result = []
    sentence_number = 0
...
def remove_sentences(pipeline, src_lang, tgt_lang, sentences):
    ...
    remove_from_conllu(folder + 'parsed/' + pipeline + "." +
                       src_lang + ".parsed.conllu", sentences)
    logging.info("Updating tgt parsed")
    remove_from_conllu(folder + 'parsed/' + pipeline + "." +
                       tgt_lang + ".parsed.conllu", sentences)
    logging.info("Updating src tokenized")
    remove_from_text(folder + 'tokenized/' + pipeline + "." +
                     src_lang + ".tokenized.txt", sentences)
    logging.info("Updating tgt tokenized")
    remove_from_text(folder + 'tokenized/' + pipeline + "." +
                     tgt_lang + ".tokenized.txt", sentences)
    logging.info("Updating aligned")
    remove_from_text(folder + 'aligned/training.align', sentences)

def postprocess(arg_pipeline):

    logging.info(f'Starting postprocessing: {arg_pipeline}')
    ...
    try:
        ...
        if arg_pipeline not in config["pipelines"]:
            msg = f'Pipeline for: {arg_pipeline} not available'
            logging.error(msg)
            raise Exception(msg)
        ...
        sentences = find_sent_to_remove(arg_pipeline, src_lang, tgt_lang)

        remove_sentences(arg_pipeline, src_lang, tgt_lang, sentences)

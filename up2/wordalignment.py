...
def process_batch(batch_data: dict) -> dict:
    ...
    if not aligner:
        ...
        aligner = SentenceAligner(
            model="bert", token_type="word", matching_methods="i", device=device)
        logging.info(
            f'Initializing aligner batch: {index}, process: {current_process}, device: {device}')
...
def word_alignment(arg_pipeline):
    config = read_config()
    ...
    src_file = tokenized + "/" + arg_pipeline + "." + src_lang + ".tokenized.txt"

    tgt_file = tokenized + "/" + arg_pipeline + "." + tgt_lang + ".tokenized.txt"
    ...
    if not batch_save:
        ...
        save_alignments(arg_pipeline, sorted_result)
